...
class Hexmove():
    def __init__(self) -> None:
        ...
        self.device_list = {
            ...
            'arm_left': {
                'can': 'can0',
                'T_robot_to_arm': np.array([[1, 0, 0, 0.32],
                                               [0, 1, 0, -0.32],
                                               [0, 0, 1, 1.0],
                                               [0, 0, 0, 1]]),
            },
    ...
    def get_arm_pose(self, commond):
        ...
        arm_pose = self.device_list[arm_id]['device'].get_arm_pose()
        return arm_pose
    ...
    def arm_move_local(self, commond):
        ...
        arm_pose = [position[0], position[1], position[2], 0, 85, 0, 0]
        self.device_list[arm_id]['device'].arm_ctrl(arm_pose)
        return 'done'

    def arm_ctrl(self, commond):
        arm_id = commond[1]
        arm_pose = commond[2]
        self.init_device(arm_id)
        self.device_list[arm_id]['device'].arm_ctrl(arm_pose)
    ...
    def init_device(self, device_id):
        if 'device' not in self.device_list[device_id]:
            ...
            elif device_id in ['FemtoBolt_down', '336L_down']:
                serial_number = self.device_list[device_id]['serial_number']
